...
class DegenerateNet(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ConvBlock(256, 128),
            ConvBlock(128, 64),
            nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)  # Output: fake IR
        )
...
def test_degenerate_net(model, image_path, output_dir=None, output_name=None):
    """
    使用退化网络对单张图片进行测试
    model: 加载好的DegenerateNet模型
    image_path: 输入图片路径
...
def batch_test_degenerate_net(model, input_dir, output_dir, prefix="ir_"):
    """
    批量测试目录中的所有图片
    """
    os.makedirs(output_dir, exist_ok=True)
    ...
    for i, img_file in enumerate(image_files):
        ...
        try:
            test_degenerate_net(model, img_path, output_dir, output_name)
...
if __name__ == "__main__":
    # 设置参数解析器
    parser = argparse.ArgumentParser(description="Test Degenerate Network")
    parser.add_argument("--model_path", type=str, default="./1checkpoints/best_degen.pth", help="Path to the trained DegenerateNet model")
    parser.add_argument("--input_path", type=str, default="./train_data/MSRS/test/vi", help="Path to input image or directory")
    parser.add_argument("--output_path", type=str, default="./degenerate_recovery_results/ir_wei", help="Path to save output image or directory")
    ...
    # 初始化模型
    degen_net = DegenerateNet().cuda()
    
    # 加载模型权重
    try:
        degen_net.load_state_dict(torch.load(args.model_path))
    ...
    if os.path.isfile(args.input_path):
        ...
        output_dir = args.output_path
        if output_dir is None:
            # 默认输出到同级目录下的"ir_results"文件夹
            output_dir = os.path.join(os.path.dirname(args.input_path), "ir_results")
        
        # 执行批量测试
        start_time = time.time()
        batch_test_degenerate_net(
            model=degen_net,
